...
import { migratePool, poolAs } from './helpers.ts';
...
describe('per-app database isolation (Section 5.5)', () => {
  ...
  before(async () => {
    ...
    await connectApp(migrator, 'thrivo', 'Thrivo', 'local_dev_only');
    await connectApp(migrator, 'toplance', 'Toplance', 'local_dev_only');
    ...
    await migrator.query(`CREATE SCHEMA IF NOT EXISTS toplance`);
    await migrator.query(
  ...
  after(async () => {
    await migrator.query(`DROP TABLE IF EXISTS thrivo.leads`);
    await migrator.query(`DROP TABLE IF EXISTS toplance.projects`);
    await migrator.end();
    await thrivo.end();
  });
