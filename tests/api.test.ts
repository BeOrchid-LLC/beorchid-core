...
describe('Core API', () => {
  ...
  describe('permission resolution (Sections 6.1a, 6.3)', () => {
    ...
    it('resolves a different set for the same user in a second app', async () => {
      // Authenticated as the OTHER app, using its own key. Section 6.4 asks for
      // this contrast; it has to be produced the way a real second app would
      // produce it, not by one app asking about another's.
      const other = await get(
        `/v1/permissions/resolve?membership_id=${fx.membershipId}`,
        OTHER_API_KEY,
      );
    ...
    it('refuses to resolve another app\'s permissions', async () => {
      // Authenticated as one app, asking about another. Without this check any
      // app's key could read any other app's resolved permissions for an
      // arbitrary membership, reintroducing above the database exactly the
      // cross-app leak the Section 5.2 views prevent inside it.
      const res = await get(
        `/v1/permissions/resolve?membership_id=${fx.membershipId}&app_id=${fx.otherAppId}`,
      );
      assert.equal(res.status, 403, 'one app resolved another app\'s permissions');

      const body = (await res.json()) as any;
      assert.match(body.error, /app_id/);
    });

    it('records the refusal, attributed to the app that attempted it', async () => {
      await db.query('DELETE FROM core.access_log');
      await get(`/v1/permissions/resolve?membership_id=${fx.membershipId}&app_id=${fx.otherAppId}`);

      const { rows } = await db.query<{ result: string; app_key: string; action: string }>(
        `SELECT l.result, l.action, a.key AS app_key
      ...
      assert.equal(rows[0]?.action, 'permissions:resolve');
      assert.equal(rows[0]?.app_key, APP_KEY);
    });

    it('accepts its own app_id, since Sections 3.1a and 5.6 document it', async () => {
      const res = await get(
        `/v1/permissions/resolve?membership_id=${fx.membershipId}&app_id=${fx.appId}`,
      );
      assert.equal(res.status, 200);
