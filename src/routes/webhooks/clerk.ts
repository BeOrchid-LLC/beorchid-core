...
clerkWebhook.post('/clerk', async (c) => {
  ...
  try {
    // verify() throws on an invalid signature, which is the security-relevant
    // behaviour and the only thing relied on here. Its RETURN value is not:
    // svix has changed whether it hands back the parsed payload across major
    // versions, so the body is parsed explicitly rather than trusting it. The
    // signature has already been checked at this point, so the bytes being
    // parsed are the bytes that were signed.
